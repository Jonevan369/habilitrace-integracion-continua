...
INPUT = ROOT / "entregafinalv5_HabiliTrace_culminado.docx"
OUTPUT = ROOT / "entrega_final_HabiliTrace_solo_ediciones.docx"
...
def para_after_element(doc, element, text=""):
...
def style_like_existing_tables(table):
...
def insert_usability_section(doc):
    ...
    cursor._p.addnext(table_element)
    ...
    for row, values in zip(table.rows, data):
        for cell, value in zip(row.cells, values):
            cell.text = value
    style_like_existing_tables(table)
